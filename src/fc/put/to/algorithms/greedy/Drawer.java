...
public class Drawer {
    ...
    public void draw(List<List<Vertex>> incidenceList) {
        ...
        for (int i = 0; i < Constants.LENGHT_OF_SOLUTION; i++) {
    ...
    public void drawNN(List<Vertex> bestSolution) {
        readFileWithCoordinates();
        this.graph = new MultiGraph("Hello World!");
        this.vertices.forEach(this::drawNewVertex);

        Vertex from = bestSolution.get(0);
        for (int i = 1; i < Constants.LENGHT_OF_SOLUTION + 1; i++) {
            System.out.print(from + " ");
            Vertex to = bestSolution.get(i);
            drawEdges(from, to);
            from = to;
        }
        graph.addAttribute("ui.stylesheet", "node { size: 7px; z-index: 0;}");
